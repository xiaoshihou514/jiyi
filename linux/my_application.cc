// https://github.com/Skyost/OpenAuthenticator/blob/152acc82369ec9b2a7564e348ab25120ad5510b0/linux/my_application.cc
// Licensed under GPL-3.0, original author Hugo Delaunay "Skyost"
...
static gchar *get_real_name() {
  ...
  GVariant *result = g_dbus_connection_call_sync(
      ...
      g_variant_new("(s)", getenv("USER")), G_VARIANT_TYPE("(o)"),
  ...
  GVariant *name_result = g_dbus_connection_call_sync(
      connection, "org.freedesktop.Accounts", user_path,
...
static int pam_conversation(int num_msg, const struct pam_message **msg,
  ...
  struct pam_response_t *data =
      static_cast<struct pam_response_t *>(appdata_ptr);
...
static void authenticate(const gchar *password, FlMethodCall *method_call) {
  ...
  if (pw == nullptr) {
    fl_method_call_respond(
        ...
        FL_METHOD_RESPONSE(fl_method_error_response_new(
            "authError", "Cannot get current user.", nullptr)),
        nullptr);
    return;
  }
  ...
  if (ret != PAM_SUCCESS) {
    fl_method_call_respond(
        method_call,
        FL_METHOD_RESPONSE(fl_method_error_response_new(
            "authError", "Could not start authentication session.", nullptr)),
        nullptr);
    return;
  }
  ...
  fl_method_call_respond(
      method_call, FL_METHOD_RESPONSE(fl_method_success_response_new(success)),
      nullptr);
}
