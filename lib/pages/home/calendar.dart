...
class _CalendarState extends State<Calendar> {
      ...
      builder: (context, counter, child) => FutureBuilder<List<Metadata>>(
        ...
        builder: (context, snapshot) {
          ...
          return Container(
            ...
            child: ListView(children: _buildMonthSections(context, 3.em)),
          );
        },
      ),
    );
  ...
  void _processRecords(List<Metadata> records) {
    ...
    // 按月份组织日期
    final dates = _dailyCovers.keys.toList()..sort();
  ...
  List<Widget> _buildMonthSections(BuildContext context, double em) {
    final monthKeys = _entriesByMonth.keys.toList()
      ..sort((a, b) => b.compareTo(a));

    return monthKeys.map((monthKey) {
      final dates = _entriesByMonth[monthKey]!..sort((a, b) => b.compareTo(a));
  ...
  Widget _buildDateStatus(List<String> covers, DateTime date) {
    return Column(
      children: [
        _buildStatusCircles(covers.sublist(0, min(covers.length, 2))),
  ...
  Widget _buildStatusCircles(List<String> covers) {
    final size = 15.em;
    final offset = 3.5.em; // 重叠偏移量
    final circleCount = covers.length;
        ...
        children: List.generate(circleCount, (index) {
          return Positioned(
            ...
            child: Container(
              ...
              height: size,
              decoration: BoxDecoration(
                color: _getStatusColor(covers[index]),
                shape: BoxShape.circle,
                border: Border.all(color: DefaultColors.bg, width: 1.em),
              ),
              child: Center(
                child: Text(covers[index], style: TextStyle(fontSize: 7.5.em)),
              ),
            ),
          );
        }),
